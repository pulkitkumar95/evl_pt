...
class MotionFuseRGBModule(nn.Module):

    def __init__(
        self,
        fea_dim_absolute = 768,
        fea_dim_selfmotion =768,
        out_feature_dim = 768,
        num_patches = 196,
    ):
        super().__init__()
        ...
        assert (fea_dim_absolute == fea_dim_selfmotion == out_feature_dim)
        

    
    def forward(self, point_trajs_gt_coord, point_trajs_visibility_mask, point_trajs_gt_rgbtoken_index, temporal_pos_embed, frame_features):
        '''
        Args:
        point_trajs_gt_coord: Tensor of shape (B, M, T, 2) -> M trajectories point coordinates, normalized between [-1, 1]
        point_trajs_visibility_mask: Tensor of shape (B, M, T) -> 1 indicates point visible, 0 indicates point invisible
        ...
        frame_features: (B, T, N, C), frame features
        Returns:
        ...
        N = frame_features.shape[2]
        assert (frame_features.shape == (B, T, N, C))
        ...
        absolute_pos_embd_out = absolute_pos_embd_out*(point_trajs_visibility_mask.permute(0,2,1).unsqueeze(3))
        assert (absolute_pos_embd_out.shape == (B, T, M, D_abs))
        ...
        ########### final motion
        motion_out_feas = absolute_pos_embd_out + selfmotion_feas + temporal_pos_embed.view(1, T, 1, C)
        assert (motion_out_feas.shape==(B, T, M, C))


        ########## fuse with RGB frame features
        indexing_for_rgb = point_trajs_gt_rgbtoken_index.permute(0,2,1).reshape(B*T, M)  # (B*T, M)
        
        frame_features_ordered = torch.gather(frame_features.flatten(0,1), 1, indexing_for_rgb.unsqueeze(-1).expand(-1, -1, C)) # frame (B, T, N, C)->(B*T, N, C), index (N*T, M) -> (B*T, M, C), output (B*T, M, C)
        frame_features_ordered = frame_features_ordered.view(B, T, M, C)


        #### fuse motion and rgb
        fuse_features_out = frame_features_ordered + motion_out_feas

        # if torch.isnan(motion_out_feas).any():
        #     breakpoint()  # Trig

        return fuse_features_out   



class EVLDecoderMotionFuseRGB(nn.Module):
    ...
    def __init__(
        ...
        self.motion_model = MotionFuseRGBModule(fea_dim_absolute = 768, fea_dim_selfmotion=768)
    ...
    def forward(self, frame_features, pt_coords, pt_visibility, pt_grid_indices):
        ...
        # motion features
        fuse_features = self.motion_model(pt_coords, pt_visibility, pt_grid_indices, self.temporal_pos_embed[0], frame_features) #(B, T, M, C)
        fuse_features = fuse_features.flatten(1, 2) # B, T * M, C

        # a transformer block
        out_features = self.blocks[0](fuse_features)

        x = self.decoder_layers[0](x, out_features)
...
class EVLTransformerMotionFuseRGB(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = EVLDecoderMotionFuseRGB(
        ...
        print('verbose...EVLTransformerMotionFuseRGB')
    ...
    def forward(self, x, pt_coords, pt_visibility, pt_grid_indices):
...
if __name__ == "__main__":
    ...
    frame_features = torch.rand(B,T,N,C)

    # Create model instance
    model = MotionFuseRGBModule(fea_dim_absolute = 768, fea_dim_selfmotion=768)

    # Test the model
    output = model(point_trajs_gt_coord, point_trajs_visibility_mask, point_trajs_gt_rgbtoken_index, temporal_pos_embed, frame_features)
