...
def check_vis(video, keypoints, keypoints_vis, keypoint_queries,index_select=None, point_to_take=None, renorm_points=True,
              gif_path=None):
    if gif_path is None:
        gif_path = 'vis.gif'
    if index_select is not None:
        keypoints = keypoints[index_select]
        keypoints_vis = keypoints_vis[index_select]
    
    keypoints = keypoints.numpy()
    if renorm_points:
        keypoints = (keypoints + 1) / 2
        height, width = video.shape[1:3]
        keypoints = keypoints * np.array([width, height]).reshape(1, 1, 2)

    frames = vis_trail(video, keypoints, keypoints_vis, keypoint_queries, point_to_take)
...
def make_vis(vid_info, keypoints, keypoints_vis, keypoint_queries, point_to_take, gif_path, use_video=None):
